...
// Events
export const onHistoryVisited = (callback: Parameters<typeof chrome.history.onVisited.addListener>[0]): () => void => {
    return handleListener(history().onVisited, callback);
}

export const onHistoryVisitRemoved = (callback: Parameters<typeof chrome.history.onVisitRemoved.addListener>[0]): () => void => {
