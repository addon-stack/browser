...
export interface BrowserMemoryState {
    ...
    reset(): void;
    /** @internal Shared lifecycle seam; unlike public events it survives controls.reset(). */
    onTabRemoved(listener: (tabId: number) => void): void;
    /** Removes state and returns cleanup failures so batch callers can finish before reporting them. */
    removeTab(tabId: number): readonly unknown[];
    nextTabId(): number;
    nextWindowId(): number;
