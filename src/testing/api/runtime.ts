...
import {createContextRegistry} from "../model/contexts";
...
export interface RuntimeHarnessOptions extends ContextRegistryOptions {
    extensionId?: string;
...
export const createRuntimeHarness = (
    ...
    const contextState = createContextRegistry(options, path => `${urlScheme}://${extensionId}/${path}`, state);
    ...
    const getContexts = createBrowserMethod<typeof chrome.runtime.getContexts, chrome.runtime.ExtensionContext[]>({
        ...
        implementation: ((
            ...
            const result = contextState.runtimeContexts(filter);
    ...
    return {
        ...
        addContext(context): void {
            contextState.addRuntimeContext(context);
        },
        emitMessage(message): Promise<unknown> {
            return dispatchMessage(message);
        },
        removeContext(contextId): void {
            contextState.registry.remove(contextId);
        },
        reset(): void {
            closeMessageChannels();
            ...
            contextState.registry.reset();
        },
        setContexts(value): void {
            contextState.setRuntimeContexts(value);
        },
